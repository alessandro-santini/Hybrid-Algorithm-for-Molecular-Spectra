...
Hsp = H.to_sparse()
t0,tf,nsteps = -int(1e5), int(1e5), 409701
...
samples_per_step = int(sys.argv[3])
...
with open(os.path.join(output_dir, f"psi_dict_{molecule}_{base_mol}_{no}o{ne}e_R{R}_nsamples{samples_per_step}_seed{SEED}.pkl"), "wb") as f:
...
G_dict = {
    'full': G,
    'proj': G_proj,
    'max_length':max(list(map(len,base_yofx))),
    'basis_evolved':max(list(map(len,base_yofx))),
    'sampled_initial':x_best
}

with open(os.path.join(output_dir, f'Samples_G_dict_{molecule}_{base_mol}_{no}o{ne}e_R{R}_nsamples{samples_per_step}_seed{SEED}.pkl'), 'wb') as f:
